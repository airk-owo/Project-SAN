...
- Engine modularized: `packages/game/src/engine/` (state, skills, sync, events, targeting, turn-flow, actions, setup, turns, view, handlers/) behind a thin `index.ts` router — **zero import cycles** since the 2026-07-08 refactor (`npx madge --circular --extensions ts packages/game/src/index.ts` must stay at 0; ARCHITECTURE.md §3 has the layer map).
- Refactor pass 2026-07-08 (all suites green throughout): shared test fixtures in `packages/game/src/test-helpers.ts` (no more copy-pasted `makeCard`/`makeGame`); `apps/web/app/page.tsx` split from 4,937 → ~3,260 lines — socket singleton (`app/lib/socket.ts`), audio cues (`app/lib/audio.ts`), and 20 components under `apps/web/components/` (`game/` holds all 13 pending-skill dialogs, ResponseWindowPanel, TargetCardPicker, JudgmentPanel, WinnerScreen, PreGameTable, TablePiles). Remaining in `Home()`: opponents ring / turn controls / mode bars / hand — deliberately kept until the ~18 `xMode` skill-states are consolidated (see Incomplete).
...
- page.tsx split, remaining steps (deferred by design 2026-07-08): consolidate the ~18 per-skill `xMode/xCards` states into a `useSkillModes` hook, then extract OpponentsRing / TurnControls / ModeBars / HandArea and the socket/banners/prefs hooks. High blast radius + zero web tests → do this with a full manual QA session (deep dialog states were extracted verbatim but only lobby rendering was re-verified; run one full match incl. สกิลที่มี dialog ก่อน deploy จริง).
