...
type Props = {
  ...
  canTarget: boolean;
  onSelectEmpty: (seatIndex: number) => void;
  ...
  if (!player) {
    return waiting ? (
      <article
        ...
        onClick={() => onSelectEmpty(seatIndex)}
        style={style}
      >
        <b>ที่นั่ง {seatIndex}</b>
  ...
  return (
    <article
      onClick={() =>
        canTarget
          ? onTarget(player)
          : player.character && onCharacterDetails(player)
      }
      className={`player-seat ${isCurrentTurn ? "active-player" : ""} ${canTarget ? "targetable" : ""} ${player.character ? "has-character" : ""}`}
      style={style}
    >
      <b>
        {crownTitle && (
          <span className="crown" title={crownTitle}>
            ♛
          </span>
        )}
        {player.username}
      ...
      {player.connectionStatus === "disconnected" ? (
        ...
        <>
          <p className="general-name">{player.character.name}</p>
          {hearts(player.hp, player.maxHp)}
          <p className="hand-backs">
            {"🂠".repeat(Math.min(player.handCount, 6))}{" "}
            <small>hand: {player.handCount}</small>
          </p>
        </>
      ) : (
        <p className={`ready-status ${player.ready ? "ready" : "not-ready"}`}>
          {player.ready ? "พร้อม" : "ไม่พร้อม"}
        </p>
      )}
    </article>
  );
