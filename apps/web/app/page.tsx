...
type Character={id:string;name:string;hp:number;kingdom?:string;kingdomTh?:string;gender?:string;image?:string|null;skills:{name:string;description:string;condition?:string|null}[]};
type Card={id:string;name:string;type:string;cardType:string;suit:string;number:string;description:string|null;effect:string|null;equipmentSlot:string|null;image?:string|null;effectParams?:{range?:number}|null};
...
type ResponseWindow={type:string;currentResponderId:string|null;status:'open'|'resolved';responses:{playerId:string;response:'card'|'decline'|'timeout';card?:Card}[];requiredPlayerIds?:string[];dyingPlayerId?:string};
...
const PHASE_LABEL:Record<string,string>={judgment:'ตัดสิน',draw:'จั่ว',play:'เล่น',discard:'ทิ้งไพ่',end:'จบเทิร์น'};
...
const HEART='♥︎';
const hearts=(hp?:number,maxHp?:number)=>hp===undefined||maxHp===undefined?null:<span className="mock-hearts">{HEART.repeat(Math.max(0,hp))}<i>{HEART.repeat(Math.max(0,maxHp-hp))}</i></span>;
...
const SUIT_TEXT:Record<string,string>={'♠':'♠︎','♥':'♥︎','♦':'♦︎','♣':'♣︎'};
const suitTx=(s:string)=>SUIT_TEXT[s]??s;
/** Short type label: drops the "อุปกรณ์ /" prefix and appends the mount distance modifier. */
const cardTypeLabel=(c:Card)=>{const base=(c.type||'').replace(/^อุปกรณ์\s*\/\s*/,'');if(c.equipmentSlot==='offensive_mount')return `${base} −1`;if(c.equipmentSlot==='defensive_mount')return `${base} +1`;return base;};
const KINGDOM_FACTION:Record<string,string>={WEI:'wei',SHU:'shu',WU:'wu',QUN:'qun'};

function EquipmentDisplay({eq,onInspect}:{eq:EquipmentSlots;onInspect?:(card:Card)=>void}){const r=(key:keyof EquipmentSlots,icon:string,label:string)=>{const s=eq[key];return <span className={`mock-equipment-slot ${s?'equipped':''}${s&&onInspect?' local-inspectable':''}`} title={s?.name??`${label}: ว่าง`} onClick={s&&onInspect?(e=>{e.stopPropagation();onInspect(s);}):undefined}><i>{icon}</i><em>{label}</em><b>{s?.name??'—'}</b></span>;};return <div className="mock-equipment">{r('weapon','🗡','อาวุธ')}{r('armor','🛡','เกราะ')}{r('offensiveMount','🐎','ม้ารุก −1')}{r('defensiveMount','🐎','ม้ารับ +1')}</div>;}
function DecisionArea({cards,onInspect}:{cards:Card[];onInspect?:(card:Card)=>void}){if(!cards?.length)return null;return <div className="local-decision-area">{cards.map(c=><span key={c.id} className={`local-decision-card${onInspect?' local-inspectable':''}`} title={c.description||c.name} onClick={onInspect?(e=>{e.stopPropagation();onInspect(c);}):undefined}>{c.effect==='delayed_lightning_judgment'?'⚡':c.effect==='delayed_skip_play_phase'?'🕒':'🎴'} {c.name}</span>)}</div>;}
...
function OpponentPanel({player,targetable,distance,onClick,onSkills,onInspect}:{player:Player;targetable?:boolean;distance?:number|null;onClick?:()=>void;onSkills?:()=>void;onInspect?:(card:Card)=>void}){
 return <article onClick={onClick} className={`mock-player local-opponent ${targetable?'local-targetable':''} ${!player.alive?'local-dead':''}`}>
  <div className="mock-portrait">{player.character?.image?<img src={player.character.image} alt={charName(player)}/>:charName(player).slice(0,1)}</div>
  <div className="mock-player-content">
   ...
   <div className="local-hp-hand">{hearts(player.hp,player.maxHp)}<span className="mock-hand-count">🂠 × {player.handCount??player.hand.length}</span></div>
   <small className="mock-seat-info">ที่นั่ง {player.seatIndex}{distance!=null?` · ระยะ ${distance}`:''}</small>
   <small className={`mock-role${player.role?' local-role-'+player.role:''}`}>บทบาท: {player.role?ROLE_LABEL[player.role]??player.role:'???'}</small>
...
export default function Home(){
 ...
 useEffect(()=>{const f=game?.tableFlash;if(!f)return;if(prevFlashAt.current===undefined){prevFlashAt.current=f.at;return;}if(prevFlashAt.current===f.at)return;prevFlashAt.current=f.at;setTableBanner(f);if(flashTimer.current)clearTimeout(flashTimer.current);flashTimer.current=setTimeout(()=>setTableBanner(undefined),4000);},[game?.tableFlash?.at]);
 ...
 const loadRooms=()=>fetch(`${process.env.NEXT_PUBLIC_SOCKET_URL||'http://localhost:3001'}/rooms`).then(r=>r.json()).then(setRooms).catch(()=>setRooms([]));
 useEffect(()=>{if(game)return;loadRooms();const id=setInterval(loadRooms,5000);return()=>clearInterval(id);},[game]);
 ...
 const hasUnlimitedAttack=myPlayer?.equipment.weapon?.effect==='unlimited_attack_per_turn'||(game.characterSkillKeys?.[game.viewerId]||[]).includes('unlimited_attack'); // หน้าไม้กล หรือ เตียวหุย คำราม → โจมตีได้ไม่จำกัด
 const canSnakeAttack=canAct&&hasSnakeSpear&&(hasUnlimitedAttack||(game.turn?.attackUsedThisTurn??0)<1);
 ...
 const canPlayCard=(card:Card)=>{
  ...
  if(card.effect==='attack')return hasUnlimitedAttack||(game.turn?.attackUsedThisTurn??0)<1;
  if(card.effect==='dodge')return myConv('attack',card)&&(hasUnlimitedAttack||(game.turn?.attackUsedThisTurn??0)<1); // จูล่ง/กวนอู: ใช้ "หลบ"/ไพ่แดง เป็น "โจมตี"
  return true;
 };
 ...
 const navbar=<nav className="local-navbar">
   ...
   <div className="local-navbar-right">
    ...
    <button className={`local-nav-btn${openPanel==='logchat'?' active':''}`} onClick={()=>setOpenPanel(p=>p==='logchat'?null:'logchat')} title="บันทึก/แชท">💬</button>
    {myPlayer?.role&&<button className="local-nav-btn" onClick={()=>setShowRole(true)} title="บทบาทของฉัน">🎭</button>}
    <button className="local-nav-btn" onClick={()=>setSoundOn(v=>!v)} title="เปิด/ปิดเสียง">{soundOn?'🔔':'🔕'}</button>
   </div>
  </nav>;
 const navPanels=<>{openPanel==='room'&&roomContent}{openPanel==='logchat'&&logChatContent}</>;
 ...
 return <>{navbar}<main className={isPlaying?`mock-game-page local-game-page mock-count-${game.players.length}`:'game-page'}>
  ...
  {drawPreview&&drawPreview.length>0&&<div className="local-draw-preview" role="status" onClick={()=>setDrawPreview(null)}>
    ...
    <p className="local-draw-preview-label">🎴 ได้รับการ์ด {drawPreview.length} ใบ · แตะเพื่อปิด</p>
   </div>}
  ...
  {isPlaying&&<section className="mock-match-layout">
   ...
   {opponents.map((player,index)=>{const pos=edgePosition(index,opponents.length);const style={'--seat-x':pos.left,'--seat-y':pos.top} as CSSProperties;const attackTarget=Boolean(selectedAttackId&&player.alive);const discardTargetable=Boolean(selectedDiscardId&&player.alive&&player.id!==game.viewerId);const stealTargetable=Boolean(selectedStealId&&player.alive&&player.id!==game.viewerId);const snakeTargetable=Boolean(snakeMode&&snakeCards.length===2&&player.alive&&player.id!==game.viewerId);const multiTargetable=Boolean(multiAttackId&&player.alive&&player.id!==game.viewerId);const coerceHolderPick=Boolean(coerceCardId&&!coerceHolderId&&player.alive&&player.id!==game.viewerId&&player.equipment.weapon);const coerceVictimPick=Boolean(coerceCardId&&coerceHolderId&&player.alive&&player.id!==coerceHolderId);const miracleTargetable=Boolean(miracleMode&&miracleCard&&player.alive&&player.hp!==undefined&&player.maxHp!==undefined&&player.hp<player.maxHp);const marriageTargetable=Boolean(marriageMode&&marriageCards.length===2&&player.alive&&player.character?.gender==='ชาย'&&player.hp!==undefined&&player.maxHp!==undefined&&player.hp<player.maxHp);const raidTargetable=Boolean(raidMode&&player.alive&&player.id!==game.viewerId&&(player.handCount??player.hand.length)>0);const benevolenceTargetable=Boolean(benevolenceMode&&benevolenceCards.length>0&&player.alive&&player.id!==game.viewerId);const redirectTargetable=Boolean(redirectMode&&redirectCard&&player.alive&&player.id!==game.viewerId&&player.id!==game.pendingAction?.actorId);const seduceTargetable=Boolean(seduceMode&&seduceCard&&player.alive&&player.id!==game.viewerId);const dischordTargetable=Boolean(dischordMode&&player.alive&&player.id!==game.viewerId);const inciteTargetable=Boolean(inciteMode&&inciteCard&&player.alive&&player.id!==game.viewerId&&player.character?.gender==='ชาย'&&player.id!==inciteFirst);const unityTgtable=Boolean(unityMode&&!unityTarget&&player.alive&&player.id!==game.viewerId);const unityAllyable=Boolean(unityMode&&unityTarget&&player.alive&&player.id!==game.viewerId&&player.id!==unityTarget&&player.character?.kingdom==='SHU');const guardianTargetable=Boolean(guardianMode&&player.alive&&player.id!==game.viewerId&&player.character?.kingdom==='WEI');const targetable=attackTarget||discardTargetable||stealTargetable||snakeTargetable||multiTargetable||coerceHolderPick||coerceVictimPick||miracleTargetable||marriageTargetable||raidTargetable||benevolenceTargetable||redirectTargetable||seduceTargetable||dischordTargetable||inciteTargetable||unityTgtable||unityAllyable||guardianTargetable;const onTarget=attackTarget?()=>{emit('card:play',{cardId:selectedAttackId,targetId:player.id});cancelSelection();}:guardianTargetable?()=>{emit('skill:guardian',{allyId:player.id});setGuardianMode(false);}:unityTgtable?()=>setUnityTarget(player.id):unityAllyable?()=>{emit('skill:unity',{targetId:unityTarget,allyId:player.id});setUnityMode(false);setUnityTarget(undefined);}:inciteTargetable?()=>{if(!inciteFirst)setInciteFirst(player.id);else{emit('skill:incite',{cardId:inciteCard,firstAttackerId:inciteFirst,secondPlayerId:player.id});setInciteMode(false);setInciteCard(undefined);setInciteFirst(undefined);}}:dischordTargetable?()=>{emit('skill:dischord',{targetId:player.id});setDischordMode(false);}:seduceTargetable?()=>{emit('skill:seduce',{cardId:seduceCard,targetId:player.id});setSeduceMode(false);setSeduceCard(undefined);}:redirectTargetable?()=>{emit('redirect:attack',{cardId:redirectCard,targetId:player.id});setRedirectMode(false);setRedirectCard(undefined);}:benevolenceTargetable?()=>{emit('skill:benevolence',{cardIds:benevolenceCards,targetId:player.id});setBenevolenceCards([]);}:raidTargetable?()=>{setRaidTargets(prev=>prev.includes(player.id)?prev.filter(x=>x!==player.id):prev.length<2?[...prev,player.id]:prev);}:miracleTargetable?()=>{emit('skill:miracle',{cardId:miracleCard,targetId:player.id});setMiracleMode(false);setMiracleCard(undefined);}:marriageTargetable?()=>{emit('skill:marriage',{cardIds:marriageCards,targetId:player.id});setMarriageMode(false);setMarriageCards([]);}:coerceHolderPick?()=>{setCoerceHolderId(player.id);}:coerceVictimPick?()=>{emit('coerce:play',{cardId:coerceCardId,weaponHolderId:coerceHolderId,victimId:player.id});cancelSelection();}:multiTargetable?()=>{setMultiTargets(prev=>prev.includes(player.id)?prev.filter(x=>x!==player.id):prev.length<3?[...prev,player.id]:prev);}:snakeTargetable?()=>{emit('weapon:snake-attack',{cardIds:snakeCards,targetId:player.id});cancelSnake();}:discardTargetable?()=>{setSelectedDiscardTargetId(player.id);setSelectedDiscardZone(undefined);}:stealTargetable?()=>{const dist=game.distances?.[player.id];const ignore=(game.characterSkillKeys?.[game.viewerId]||[]).includes('trick_ignore_distance');if(!ignore&&(dist==null||dist>1)){setError(`${charName(player)} อยู่นอกระยะลอบขโมย (ต้องระยะ 1) — เลือกเป้าหมายใหม่`);return;}setSelectedStealTargetId(player.id);setSelectedStealZone(undefined);}:undefined;const dist=game.distances?.[player.id];const respCard=rw?.responses?.find(r=>r.playerId===player.id&&r.response==='card')?.card;return <div key={player.id} className={`mock-opponent${multiTargets.includes(player.id)||raidTargets.includes(player.id)||inciteFirst===player.id||unityTarget===player.id?' local-multi-selected':''}`} style={style}><OpponentPanel player={player} targetable={targetable} distance={dist} onClick={onTarget} onSkills={player.character?()=>setSkillsCharacter(player.character):undefined} onInspect={setDetailCard}/>{respCard&&<div className={`local-seat-play mock-card-suit-${suitColor(respCard.suit)}`}><span className="mock-card-rank">{respCard.number}{suitTx(respCard.suit)}</span><b>{respCard.name}</b></div>}</div>;})}
   </section>
  </section>}

  {isPlaying&&<section className="mock-current-player">
   <article className="mock-player mock-self">
    <div className="mock-portrait">{myPlayer?.character?.image?<img src={myPlayer.character.image} alt={charName(myPlayer)}/>:charName(myPlayer).slice(0,1)}</div>
    <div className="mock-player-content">
     ...
     <div className="local-hp-hand">{hearts(myPlayer?.hp,myPlayer?.maxHp)}<span className="mock-hand-count">🂠 × {myPlayer?.hand.length}</span></div>
     <small className="mock-seat-info">ที่นั่ง {myPlayer?.seatIndex}</small>
     {myPlayer?.role&&<small className={`mock-role${myPlayer.role==='emperor'||roleVisible?' local-role-'+myPlayer.role:''}`}>{myPlayer.role!=='emperor'&&<button className="local-role-toggle" onClick={()=>setRoleVisible(v=>!v)} title={roleVisible?'ซ่อนบทบาท':'แสดงบทบาท'}>{roleVisible?'👁':'🙈'}</button>}บทบาท: {myPlayer.role==='emperor'||roleVisible?ROLE_LABEL[myPlayer.role]??myPlayer.role:'???'}</small>}
     <EquipmentDisplay eq={myPlayer?.equipment??{weapon:null,armor:null,offensiveMount:null,defensiveMount:null}} onInspect={setDetailCard}/>
     <DecisionArea cards={myPlayer?.decisionArea??[]} onInspect={setDetailCard}/>
    </div>
   </article>
   <div className="mock-your-turn">{myOwedDraws>0?<strong>⬆ คุณได้รับไพ่ {myOwedDraws} ใบ — กดกองจั่วเพื่อรับ</strong>:selectedAttackId||selectedDiscardId||selectedStealId?<strong>เลือกผู้เล่นเป้าหมายบนโต๊ะ</strong>:isDrawPhase?<strong>⬆ กดกองจั่วบนโต๊ะเพื่อจั่วไพ่</strong>:null}</div>
   <div className="local-turn-controls">
    ...
    {canAct&&hasMySkill('self_damage_draw')&&(myPlayer?.hp??0)>0&&<button className="local-skill-btn" onClick={()=>setConfirmSelfDamage(true)}>✦ พลีชีพ (จ่าย 1 HP จั่ว 2)</button>}
    {canAct&&hasMySkill('discard_then_draw_equal')&&!skillUsed('discard_then_draw_equal')&&!balanceMode&&!discardLimitMode&&!snakeMode&&!miracleMode&&<button className="local-skill-btn" onClick={()=>{cancelSelection();setBalanceMode(true);setBalanceCards([])}}>✦ ถ่วงดุล (ทิ้ง→จั่ว)</button>}
    {canAct&&hasMySkill('miracle_medicine')&&!skillUsed('miracle_medicine')&&!balanceMode&&!discardLimitMode&&!snakeMode&&!miracleMode&&!marriageMode&&<button className="local-skill-btn" onClick={()=>{cancelSelection();setMiracleMode(true);setMiracleCard(undefined)}}>✦ ยาสวรรค์ (ทิ้ง 1 → ฟื้น 1 HP)</button>}
    {canAct&&hasMySkill('marriage_heal')&&!skillUsed('marriage_heal')&&!balanceMode&&!discardLimitMode&&!snakeMode&&!miracleMode&&!marriageMode&&<button className="local-skill-btn" onClick={()=>{cancelSelection();setMarriageMode(true);setMarriageCards([])}}>✦ แผนแต่งงาน (ทิ้ง 2 → ฟื้น 2)</button>}
    {canAct&&hasMySkill('benevolence_give')&&!balanceMode&&!discardLimitMode&&!snakeMode&&!miracleMode&&!marriageMode&&!benevolenceMode&&<button className="local-skill-btn" onClick={()=>{cancelSelection();setBenevolenceMode(true);setBenevolenceCards([])}}>✦ เมตตาธรรม (มอบไพ่ให้พันธมิตร)</button>}
    {canAct&&hasMySkill('black_as_dismantle')&&!bandinMode&&!balanceMode&&!discardLimitMode&&!snakeMode&&<button className="local-skill-btn" onClick={()=>{cancelSelection();setBandinMode(true)}}>✦ บ้าบิ่น (♠/♣ เป็น ถอนสะพาน)</button>}
    {canAct&&hasMySkill('diamond_as_indulgence')&&!seduceMode&&!balanceMode&&!discardLimitMode&&!snakeMode&&<button className="local-skill-btn" onClick={()=>{cancelSelection();setSeduceMode(true);setSeduceCard(undefined)}}>✦ โปรยเสน่ห์ (♦ เป็น มีสุขลืมเมือง)</button>}
    {canAct&&hasMySkill('dischord')&&!skillUsed('dischord')&&(myPlayer?.hand.length??0)>0&&!dischordMode&&<button className="local-skill-btn" onClick={()=>{cancelSelection();setDischordMode(true)}}>✦ บาดหมาง (เลือกเป้าให้ทายดอก)</button>}
    {canAct&&hasMySkill('incite_duel')&&!skillUsed('incite')&&(myPlayer?.hand.length??0)>0&&!inciteMode&&<button className="local-skill-btn" onClick={()=>{cancelSelection();setInciteMode(true);setInciteCard(undefined);setInciteFirst(undefined)}}>✦ สาวงามยุยง (ให้ 2 ชายท้าสู้กัน)</button>}
    {canAct&&hasMySkill('ask_shu_attack')&&myPlayer?.role==='emperor'&&!unityMode&&<button className="local-skill-btn" onClick={()=>{cancelSelection();setUnityMode(true);setUnityTarget(undefined)}}>✦ คุณธรรมสามัคคี (ให้จ๊กก๊กโจมตีแทน)</button>}
    {isDrawPhase&&hasMySkill('raid_draw_phase')&&(game.turn?.drawnThisTurn??0)===0&&!raidMode&&<button className="local-skill-btn" onClick={()=>{cancelSelection();setRaidMode(true);setRaidTargets([])}}>✦ จู่โจมฉับพลัน (แทนการจั่ว)</button>}
    {isDrawPhase&&hasMySkill('unarmed_tiger')&&(game.turn?.drawnThisTurn??0)===0&&<button className="local-skill-btn" onClick={()=>emit('skill:hunt')}>✦ ฆ่าเสือมือเปล่า (จั่ว 1 → +1 ดาเมจ)</button>}
    {isDrawPhase&&hasMySkill('fortune_judgment')&&(game.turn?.drawnThisTurn??0)===0&&!skillUsed('fortune_done')&&<button className="local-skill-btn" onClick={()=>emit('skill:fortune')}>✦ พึ่งวาสนา (เปิดดวง เก็บดอกดำ)</button>}
    {isDrawPhase&&hasMySkill('emperor_arrogance')&&myPlayer?.role==='emperor'&&(game.turn?.drawnThisTurn??0)===0&&!skillUsed('arrogance')&&<button className="local-skill-btn" onClick={()=>emit('skill:arrogance')}>✦ จองหอง (จั่ว +1, มือ -1)</button>}
    {isDrawPhase&&hasMySkill('peek_reorder_deck')&&(game.turn?.drawnThisTurn??0)===0&&!skillUsed('peek')&&!game.pendingPeek&&<button className="local-skill-btn" onClick={()=>{setPeekOrder([]);emit('skill:peek')}}>✦ หยั่งรู้ฟ้าดิน (ดู/จัดกองจั่ว)</button>}
   </div>
   ...
   {miracleMode&&<div className="local-discard-limit-bar"><span>ยาสวรรค์: {!miracleCard?'เลือกไพ่บนมือ 1 ใบเพื่อทิ้ง':'เลือกเป้าหมายที่บาดเจ็บบนโต๊ะ'}</span>{miracleCard&&(myPlayer?.hp??0)<(myPlayer?.maxHp??0)&&<button onClick={()=>{emit('skill:miracle',{cardId:miracleCard,targetId:game.viewerId});setMiracleMode(false);setMiracleCard(undefined);}}>ฟื้นฟูตัวเอง</button>}<button className="mock-muted-button" onClick={()=>{setMiracleMode(false);setMiracleCard(undefined);}}>ยกเลิก</button></div>}
   {marriageMode&&<div className="local-discard-limit-bar"><span>แผนแต่งงาน: เลือกไพ่ทิ้ง <b>{marriageCards.length}</b>/2 ใบ{marriageCards.length===2?' → เลือกขุนพลชายที่บาดเจ็บบนโต๊ะ':''}</span><button className="mock-muted-button" onClick={()=>{setMarriageMode(false);setMarriageCards([]);}}>ยกเลิก</button></div>}
   {raidMode&&<div className="local-discard-limit-bar"><span>จู่โจมฉับพลัน: เลือกขุนพลอื่น <b>{raidTargets.length}</b>/2 คนบนโต๊ะ (แทนการจั่ว)</span><button disabled={raidTargets.length<1} onClick={()=>{emit('skill:raid',{targetIds:raidTargets});setRaidMode(false);setRaidTargets([]);}}>หยิบไพ่ ({raidTargets.length})</button><button className="mock-muted-button" onClick={()=>{setRaidMode(false);setRaidTargets([]);}}>ยกเลิก</button></div>}
   {benevolenceMode&&<div className="local-discard-limit-bar"><span>เมตตาธรรม: เลือกไพ่ <b>{benevolenceCards.length}</b> ใบ{benevolenceCards.length>0?' → เลือกผู้รับบนโต๊ะ':' (มอบ 2+ ใบในรอบ = ฟื้น 1 HP)'}</span><button className="mock-muted-button" onClick={()=>{setBenevolenceMode(false);setBenevolenceCards([]);}}>เสร็จสิ้น</button></div>}
   {bandinMode&&<div className="local-discard-limit-bar"><span>บ้าบิ่น: เลือกไพ่ ♠/♣ 1 ใบ เพื่อใช้เป็น "ถอนสะพาน"</span><button className="mock-muted-button" onClick={()=>setBandinMode(false)}>ยกเลิก</button></div>}
   {seduceMode&&<div className="local-discard-limit-bar"><span>โปรยเสน่ห์: {!seduceCard?'เลือกไพ่ ♦ 1 ใบ':'เลือกเป้าหมายบนโต๊ะ (ใช้เป็น มีสุขลืมเมือง)'}</span><button className="mock-muted-button" onClick={()=>{setSeduceMode(false);setSeduceCard(undefined);}}>ยกเลิก</button></div>}
   {dischordMode&&<div className="local-discard-limit-bar"><span>บาดหมาง: เลือกขุนพลบนโต๊ะ 1 คน</span><button className="mock-muted-button" onClick={()=>setDischordMode(false)}>ยกเลิก</button></div>}
   {inciteMode&&<div className="local-discard-limit-bar"><span>สาวงามยุยง: {!inciteCard?'เลือกไพ่ทิ้ง 1 ใบ':!inciteFirst?'เลือกขุนพลชายที่โจมตีก่อน':'เลือกขุนพลชายคู่ต่อสู้'}</span><button className="mock-muted-button" onClick={()=>{setInciteMode(false);setInciteCard(undefined);setInciteFirst(undefined);}}>ยกเลิก</button></div>}
   {unityMode&&<div className="local-discard-limit-bar"><span>คุณธรรมสามัคคี: {!unityTarget?'เลือกเป้าหมายโจมตี (ในระยะของคุณ)':'เลือกพันธมิตรจ๊กก๊กให้โจมตีแทน'}</span><button className="mock-muted-button" onClick={()=>{setUnityMode(false);setUnityTarget(undefined);}}>ยกเลิก</button></div>}
   <div className="mock-hand" ref={handRef}>{myPlayer?.hand.map(card=>{const info=cardInfo(card);const isDiscardSelected=discardLimitSelected.includes(card.id);const isSnakeSelected=snakeCards.includes(card.id);const isBalanceSelected=balanceCards.includes(card.id);const isMiracleSelected=miracleMode&&miracleCard===card.id;const isMarriageSelected=marriageCards.includes(card.id);const isBenevolenceSelected=benevolenceCards.includes(card.id);const isSeduceSelected=seduceCard===card.id;const isInciteSelected=inciteCard===card.id;const pickMode=discardLimitMode||snakeMode||balanceMode||miracleMode||marriageMode||benevolenceMode||bandinMode||seduceMode||inciteMode;const handleClick=discardLimitMode?()=>toggleDiscardLimit(card.id):snakeMode?()=>toggleSnake(card.id):balanceMode?()=>toggleBalance(card.id):miracleMode?()=>setMiracleCard(card.id):marriageMode?()=>setMarriageCards(prev=>prev.includes(card.id)?prev.filter(x=>x!==card.id):prev.length<2?[...prev,card.id]:prev):benevolenceMode?()=>setBenevolenceCards(prev=>prev.includes(card.id)?prev.filter(x=>x!==card.id):[...prev,card.id]):bandinMode?()=>{if(card.suit==='♠'||card.suit==='♣'){setSelectedDiscardId(card.id);setBandinMode(false);}}:seduceMode?()=>{if(card.suit==='♦')setSeduceCard(card.id);}:inciteMode?()=>setInciteCard(card.id):()=>selectCard(card);const playable=canPlayCard(card);return <article key={card.id} tabIndex={0} onClick={handleClick} onKeyDown={e=>{if(e.key==='Enter'||e.key===' '){e.preventDefault();handleClick();}}} className={`mock-card mock-card-suit-${suitColor(card.suit)} ${pickMode?'local-hand-card':playable?'local-hand-card':'local-card-disabled'} ${!pickMode&&(selectedAttackId===card.id||selectedDiscardId===card.id||selectedStealId===card.id)?'selected-card':''} ${(discardLimitMode&&!isDiscardSelected)||(snakeMode&&!isSnakeSelected)||(balanceMode&&!isBalanceSelected)||(miracleMode&&!isMiracleSelected)||(marriageMode&&!isMarriageSelected)||(benevolenceMode&&!isBenevolenceSelected)||(seduceMode&&!isSeduceSelected)||(inciteMode&&!isInciteSelected)?'local-discard-unselected':''} ${isDiscardSelected||isSnakeSelected||isBalanceSelected||isMiracleSelected||isMarriageSelected||isBenevolenceSelected||isSeduceSelected||isInciteSelected?'local-discard-selected':''}`}><header><span className="mock-card-rank">{card.number}{suitTx(card.suit)}</span></header><button className="local-card-info" onClick={e=>{e.stopPropagation();setDetailCard(card);}} title="ดูรายละเอียดการ์ด">ℹ</button>{card.image?<img className="mock-card-art" src={card.image} alt={card.name}/>:<div className="mock-card-art">WTK</div>}<b className="mock-card-name">{card.name}</b><small>{cardTypeLabel(card)}</small>{(isDiscardSelected||isSnakeSelected||isBalanceSelected||isMiracleSelected||isMarriageSelected)&&<span className="local-discard-badge">ทิ้ง</span>}{isBenevolenceSelected&&<span className="local-discard-badge">มอบ</span>}{isSeduceSelected&&<span className="local-discard-badge">เสน่ห์</span>}{!pickMode&&info&&<div className="card-tip" role="tooltip"><b className="card-tip-name">{card.name}</b><span className="card-tip-type">{cardTypeLabel(card)} · {card.number}{suitTx(card.suit)}</span><p className="card-tip-desc">{info.desc}</p>{info.use&&<p className="card-tip-use"><i>เมื่อไหร่:</i> {info.use}</p>}</div>}</article>;})}
   </div>
  </section>}
  ...
  {isPlaying&&rw&&rw.type!=='coerce_attack'&&rw.type!=='harvest_pick'&&<section className="mock-response" role="dialog"><span className="mock-response-icon">{rw.type==='negate'?'🛡':rw.type==='dying_heal'?'✚':'⚔'}</span>{countdownBadge}
   {rw.type==='negate'?<>{canRespond?<><div><h2>ใช้คงกระพันชาตรีหรือไม่?</h2></div><div className="mock-response-actions"><button disabled={!myPlayer?.hand.some(c=>c.effect==='negate_trick_effect')} onClick={()=>{const c=myPlayer?.hand.find(c=>c.effect==='negate_trick_effect');if(c)emit('negate:play',{cardId:c.id})}}>🛡 ใช้คงกระพันชาตรี</button><button className="mock-muted-button" onClick={()=>emit('negate:decline')}>ไม่ใช้</button></div></>:<><div><h2>กำลังรอ {responder?.username??'ผู้เล่น'}</h2><p>ว่าจะยกเลิกไพ่อุบายหรือไม่</p></div></>}</>
   :rw.type==='dying_heal'?<>{canRespond?<><div><small>{charName(dyingPlayer)} อยู่ในสถานะใกล้ตาย</small><h2>ใช้ เสบียง ช่วยหรือไม่?</h2></div><div className="mock-response-actions"><button disabled={!myPlayer?.hand.some(c=>myConv('heal',c))} onClick={()=>{const c=myPlayer?.hand.find(c=>myConv('heal',c));if(c)emit('response:heal',{cardId:c.id})}}>✚ ใช้ เสบียง</button><button className="mock-muted-button" onClick={()=>emit('response:decline')}>ไม่ทำอะไร</button></div></>:<><div><h2>กำลังรอ {responder?.username??'ผู้เล่น'}</h2><p>ว่าจะช่วย {charName(dyingPlayer)} หรือไม่</p></div></>}</>
   :<>{canRespond?<><div><h2>{rw.type==='mass_dodge'||rw.type==='multi_attack'?'ต้องใช้ หลบ':rw.type==='mass_attack'?'ต้องใช้ โจมตี':rw.type==='duel_attack'?'ท้าสู้ — ตอบโต้ด้วย โจมตี':(game.pendingAction?.dodgesRequired??1)>1?`ถูกโจมตี — ต้องใช้ หลบ อีก ${Math.max(1,(game.pendingAction?.dodgesRequired??1)-(rw.responses?.filter(r=>r.playerId===game.viewerId&&r.response==='card').length??0))} ใบ`:'ถูกโจมตี — ตอบสนอง'}</h2></div><div className="mock-response-actions">{rw.type==='attack_dodge'?<>{!game.pendingAction?.noDodge&&<button disabled={!myPlayer?.hand.some(c=>myConv('dodge',c))} onClick={()=>{const c=myPlayer?.hand.find(c=>c.effect==='dodge')||myPlayer?.hand.find(c=>myConv('dodge',c));if(c)emit('attack:respond',{cardId:c.id})}}>🛡 ใช้ หลบ</button>}<button className="mock-muted-button" onClick={()=>emit('attack:respond')}>{game.pendingAction?.noDodge?'รับความเสียหาย (หลบไม่ได้ — ม้าคะนองศึก)':'รับความเสียหาย'}</button>{myKeys.includes('redirect_attack')&&(myPlayer?.hand.length??0)>0&&!redirectMode&&<button className="local-skill-btn" onClick={()=>{setRedirectMode(true);setRedirectCard(undefined);}}>↪ ระเหเร่ร่อน (เปลี่ยนเป้า)</button>}{myKeys.includes('ask_wei_dodge')&&myPlayer?.role==='emperor'&&!game.pendingAction?.noDodge&&!guardianMode&&<button className="local-skill-btn" onClick={()=>setGuardianMode(true)}>🛡 ปกป้องราชันย์ (ให้วุยก๊กหลบแทน)</button>}{guardianMode&&<span className="local-force-cards">เลือกพันธมิตรวุยก๊กบนโต๊ะ <button className="mock-muted-button" onClick={()=>setGuardianMode(false)}>ยกเลิก</button></span>}{redirectMode&&<div className="local-force-cards">{!redirectCard?<>{myPlayer?.hand.map(c=><button key={c.id} onClick={()=>setRedirectCard(c.id)}>ทิ้ง {c.name} ({c.number}{suitTx(c.suit)})</button>)}</>:<span>เลือกเป้าหมายใหม่บนโต๊ะ (ในระยะของคุณ ไม่ใช่ผู้โจมตี)</span>}<button className="mock-muted-button" onClick={()=>{setRedirectMode(false);setRedirectCard(undefined);}}>ยกเลิก</button></div>}</>:rw.type==='mass_dodge'||rw.type==='multi_attack'?<><button disabled={!myPlayer?.hand.some(c=>myConv('dodge',c))} onClick={()=>{const c=myPlayer?.hand.find(c=>c.effect==='dodge')||myPlayer?.hand.find(c=>myConv('dodge',c));if(c)emit('mass:respond',{cardId:c.id})}}>🛡 ใช้ หลบ</button><button className="mock-muted-button" onClick={()=>emit('mass:decline')}>รับความเสียหาย</button></>:rw.type==='duel_attack'?<><button disabled={!myPlayer?.hand.some(c=>myConv('attack',c))} onClick={()=>{const c=myPlayer?.hand.find(c=>c.effect==='attack')||myPlayer?.hand.find(c=>myConv('attack',c));if(c)emit('duel:respond',{cardId:c.id})}}>⚔ ตอบโต้ด้วย โจมตี</button><button className="mock-muted-button" onClick={()=>emit('response:decline')}>ยอมแพ้ (เสีย 1 HP)</button></>:<><button disabled={!myPlayer?.hand.some(c=>myConv('attack',c))} onClick={()=>{const c=myPlayer?.hand.find(c=>c.effect==='attack')||myPlayer?.hand.find(c=>myConv('attack',c));if(c)emit('mass:respond',{cardId:c.id})}}>⚔ ใช้ โจมตี</button><button className="mock-muted-button" onClick={()=>emit('mass:decline')}>ไม่ใช้ (เสีย 1 HP)</button></>}</div></>:<><div><h2>กำลังรอ {responder?.username??'ผู้เล่น'} ตอบสนอง</h2></div></>}</>}
  </section>}

  {isPlaying&&game.pendingCoerce&&(()=>{const pc=game.pendingCoerce!;const holder=game.players.find(p=>p.id===pc.weaponHolderId);const victim=game.players.find(p=>p.id===pc.victimId);const actor=game.players.find(p=>p.id===pc.actorId);const isHolder=pc.weaponHolderId===game.viewerId;const atk=myPlayer?.hand.find(c=>c.effect==='attack');return <section className="mock-response" role="dialog"><span className="mock-response-icon">🗡</span>{countdownBadge}{isHolder?<><div><small>{charName(actor)} ใช้ {pc.trickName}</small><h2>ถูกบังคับให้โจมตี {charName(victim)}</h2></div><div className="mock-response-actions"><button disabled={!atk} onClick={()=>atk&&emit('coerce:attack',{cardId:atk.id})}>{atk?`⚔ โจมตี ${charName(victim)}`:'ไม่มีไพ่โจมตี'}</button><button className="mock-muted-button" onClick={()=>emit('coerce:decline')}>ไม่โจมตี (ให้ {charName(actor)} ยึดอาวุธ)</button></div></>:<><div><h2>กำลังรอ {holder?.username??'ผู้เล่น'} ตัดสินใจ</h2><p>ยืมมือสังหาร: โจมตี {charName(victim)} หรือเสียอาวุธ</p></div></>}</section>;})()}
  {isPlaying&&game.pendingHarvest&&rw?.type==='harvest_pick'&&(()=>{const picker=game.players.find(p=>p.id===rw.currentResponderId);const isPicker=rw.currentResponderId===game.viewerId;return <section className="mock-response local-harvest" role="dialog"><span className="mock-response-icon">🌾</span>{countdownBadge}<div><h2>{isPicker?'เลือกไพ่ 1 ใบจากยุ้งฉาง':`กำลังรอ ${picker?.username??'ผู้เล่น'} เลือกไพ่`}</h2></div><div className="local-harvest-pool">{game.pendingHarvest!.revealed.map((c,i)=>{const hidden=c.effect==='hidden_harvest';const hinfo=hidden?null:cardInfo(c);return <button key={c.id||i} disabled={!isPicker||hidden} className={`mock-card ${hidden?'local-harvest-hidden':`mock-card-suit-${suitColor(c.suit)}`}`} onClick={()=>isPicker&&!hidden&&emit('harvest:pick',{cardId:c.id})}>{hidden?null:<><header><span className="mock-card-rank">{c.number}{suitTx(c.suit)}</span></header>{c.image?<img className="mock-card-art" src={c.image} alt={c.name}/>:<div className="mock-card-art">WTK</div>}<b className="mock-card-name">{c.name}</b><small>{cardTypeLabel(c)}</small></>}{hinfo&&<div className="card-tip" role="tooltip"><b className="card-tip-name">{c.name}</b><span className="card-tip-type">{cardTypeLabel(c)} · {c.number}{suitTx(c.suit)}</span><p className="card-tip-desc">{hinfo.desc}</p>{hinfo.use&&<p className="card-tip-use"><i>เมื่อไหร่:</i> {hinfo.use}</p>}</div>}</button>;})}</div></section>;})()}
  {isPlaying&&pj&&(()=>{const jp=game.players.find(p=>p.id===pj.playerId);const r=pj.revealed;return <section className="mock-response local-judgment-panel" role="dialog"><span className="mock-response-icon">⚖</span>{countdownBadge}{pj.stage==='awaiting_draw'?<div><h2>การตัดสิน: {pj.trickName}</h2>{myJudgmentDraw?<p>⬆ กดกองจั่ว (หรือปุ่มด้านล่าง) เพื่อเปิดไพ่ตัดสิน</p>:<p>กำลังรอ {jp?.username??'ผู้เล่น'} เปิดไพ่ตัดสิน…</p>}{myJudgmentDraw&&<div className="mock-response-actions"><button onClick={()=>emit('judgment:draw')}>⚖ เปิดไพ่ตัดสิน</button></div>}</div>:<><div className="local-judgment-reveal"><small>ไพ่ตัดสินของ {charName(jp)} — {pj.trickName}</small>{r?<div className={`local-judgment-card local-suit-${suitColor(r.suit)}`}><span>{r.number} {suitTx(r.suit)}</span><b>{r.name}</b></div>:<p>ไม่มีไพ่ตัดสิน</p>}</div>{myJudgmentAct?<div className="mock-response-actions">{myKeys.includes('keep_judgment')&&<button onClick={()=>emit('judgment:keep')}>🔮 เก็บไพ่ตัดสิน (คาดการณ์แม่นยำ)</button>}<button className="mock-muted-button" onClick={()=>emit('judgment:resolve')}>{myKeys.includes('keep_judgment')?'ดำเนินการต่อ (เก็บไพ่เข้ามือ)':'ดำเนินการต่อ (ทิ้ง)'}</button></div>:<p>กำลังรอ {jp?.username??'ผู้เล่น'} ตัดสินใจ…</p>}{myGuicai&&(myPlayer?.hand.length??0)>0&&(guicaiPicking?<div className="local-force-cards">{myPlayer?.hand.map(c=><button key={c.id} onClick={()=>{emit('judgment:replace',{cardId:c.id});setGuicaiPicking(false);}}>{c.name} ({c.number}{suitTx(c.suit)})</button>)}<button className="mock-muted-button" onClick={()=>setGuicaiPicking(false)}>ยกเลิก</button></div>:<button className="local-skill-btn" onClick={()=>setGuicaiPicking(true)}>🃏 กำหนดชะตา (เปลี่ยนไพ่ตัดสิน)</button>)}</>}{pj.playerId===game.viewerId&&pj.stage==='awaiting_draw'&&hasMySkill('fortune_judgment')&&!skillUsed('fortune_done')&&<button className="local-skill-btn" onClick={()=>emit('skill:fortune')}>✦ พึ่งวาสนา (ใช้ก่อนตัดสิน — เปิดดวง เก็บดอกดำ)</button>}{pj.playerId===game.viewerId&&pj.stage==='awaiting_draw'&&pj.trickEffect==='delayed_lightning_judgment'&&myPlayer?.hand.some(c=>c.effect==='negate_trick_effect')&&<button className="local-skill-btn" onClick={()=>{const neg=myPlayer?.hand.find(c=>c.effect==='negate_trick_effect');if(neg)emit('lightning:negate',{cardId:neg.id});}}>🛡 คงกระพันชาตรี (ยกเลิกฟ้าลงโทษ)</button>}</section>;})()}
  ...
  {isPlaying&&game.pendingLegacy&&(()=>{const isOwner=game.pendingLegacy!.ownerId===game.viewerId;const owner=game.players.find(p=>p.id===game.pendingLegacy!.ownerId);if(!isOwner)return <section className="mock-response" role="dialog"><span className="mock-response-icon">📜</span>{countdownBadge}<div><h2>คำสั่งเสีย</h2><p>กำลังรอ {owner?.username??'ผู้เล่น'} มอบไพ่ให้ผู้เล่น…</p></div></section>;const cards=game.pendingLegacy!.cards;return <section className="mock-response local-legacy" role="dialog"><span className="mock-response-icon">📜</span>{countdownBadge}<div className="local-legacy-body"><h2>คำสั่งเสีย — มอบไพ่ให้ผู้เล่น</h2><p>เลือกผู้รับของแต่ละใบ (ใบบนสุดจั่วก่อน)</p><div className="local-legacy-list">{cards.map(c=><div key={c.id} className="local-legacy-row"><b className={`local-legacy-card local-suit-${suitColor(c.suit)}`}>{c.name} · {c.number}{suitTx(c.suit)}</b><div className="local-legacy-targets">{game.players.filter(p=>p.alive).map(p=><button key={p.id} onClick={()=>emit('legacy:assign',{cardId:c.id,targetId:p.id})}>{p.username}{p.id===game.viewerId?' (ตัวเอง)':''}</button>)}</div></div>)}</div></div></section>;})()}
  {isPlaying&&game.pendingRetaliateJudgment&&(()=>{const owner=game.players.find(p=>p.id===game.pendingRetaliateJudgment!.ownerId);const damager=game.players.find(p=>p.id===game.pendingRetaliateJudgment!.damagerId);const isOwner=game.pendingRetaliateJudgment!.ownerId===game.viewerId;return <section className="local-repeat-attack" role="dialog"><b>🩸 ย้อนรอยศัตรู{isOwner?'':` — ${owner?.username??'ผู้เล่น'}`}</b>{countdownBadge}{isOwner?<><p>เปิดไพ่ตัดสินเอง — ถ้าไม่ใช่ ♥ ผู้ทำดาเมจ ({damager?.username??'ผู้เล่น'}) ต้องทิ้งไพ่ 2 ใบ หรือรับความเสียหาย 1 หน่วย</p><div className="mock-response-actions"><button onClick={()=>emit('retaliate:reveal')}>🎴 เปิดไพ่ตัดสิน</button></div></>:<p>กำลังรอ {owner?.username??'ผู้เล่น'} เปิดไพ่ตัดสิน ย้อนรอยศัตรู…</p>}</section>;})()}
  {isPlaying&&game.pendingRetaliate&&game.pendingRetaliate.damagerId===game.viewerId&&(()=>{const victim=game.players.find(p=>p.id===game.pendingRetaliate!.victimId);const toggle=(id:string)=>setRetaliateCards(prev=>prev.includes(id)?prev.filter(x=>x!==id):prev.length<2?[...prev,id]:prev);const canDiscard=(myPlayer?.hand.length??0)>=2;return <section className="local-repeat-attack" role="dialog"><b>🩸 คุณถูกสกิล ย้อนรอยศัตรู ของ {victim?.username??'ผู้เล่น'} — ทิ้งไพ่บนมือ 2 ใบ หรือรับความเสียหาย 1 หน่วย</b>{countdownBadge}{canDiscard&&<div className="local-force-cards">{myPlayer?.hand.map(c=><button key={c.id} className={retaliateCards.includes(c.id)?'selected-card':''} onClick={()=>toggle(c.id)}>{c.name} ({c.number}{suitTx(c.suit)})</button>)}</div>}<div className="mock-response-actions">{canDiscard&&<button disabled={retaliateCards.length!==2} onClick={()=>{emit('retaliate:discard',{cardIds:retaliateCards});setRetaliateCards([]);}}>ทิ้งไพ่ 2 ใบ ({retaliateCards.length}/2)</button>}<button className="mock-muted-button" onClick={()=>{emit('retaliate:damage');setRetaliateCards([]);}}>รับความเสียหาย 1 หน่วย</button></div></section>;})()}
  {isPlaying&&game.pendingDischord&&game.pendingDischord.targetId===game.viewerId&&(()=>{const jiuyi=game.players.find(p=>p.id===game.pendingDischord!.jiuyiId);return <section className="local-repeat-attack" role="dialog"><b>🎴 บาดหมาง จาก {charName(jiuyi)}: เลือก 1 ดอกไพ่ (ทายผิดดอกที่หยิบได้ = เสีย 1 HP แต่ได้ไพ่ใบนั้น)</b>{countdownBadge}<div className="mock-response-actions">{['♠','♥','♦','♣'].map(s=><button key={s} onClick={()=>emit('dischord:suit',{suit:s})}>{s}</button>)}</div></section>;})()}
  {isPlaying&&game.pendingAllyAssist&&game.pendingAllyAssist.allyId===game.viewerId&&(()=>{const pa=game.pendingAllyAssist!;const emperor=game.players.find(p=>p.id===pa.emperorId);const need=pa.kind;const card=myPlayer?.hand.find(c=>c.effect===need);return <section className="local-repeat-attack" role="dialog"><b>👑 {charName(emperor)} ขอให้คุณใช้ {need==='attack'?'โจมตี':'หลบ'} แทน{pa.targetId?` (เป้าหมาย ${charName(game.players.find(p=>p.id===pa.targetId))})`:''}</b>{countdownBadge}<div className="mock-response-actions"><button disabled={!card} onClick={()=>card&&emit('ally:assist',{cardId:card.id})}>{card?`${need==='attack'?'⚔ ใช้ โจมตี':'🛡 ใช้ หลบ'} แทน`:`ไม่มีไพ่${need==='attack'?'โจมตี':'หลบ'}`}</button><button className="mock-muted-button" onClick={()=>emit('ally:decline')}>ปฏิเสธ</button></div></section>;})()}
  {isPlaying&&game.pendingPeek&&game.pendingPeek.playerId===game.viewerId&&(()=>{const cards=game.pendingPeek.cards;const toggle=(id:string)=>setPeekOrder(prev=>prev.includes(id)?prev.filter(x=>x!==id):[...prev,id]);const unselected=cards.filter(c=>!peekOrder.includes(c.id));const bottomIds=unselected.map(c=>c.id);const ordered=[...peekOrder.map(id=>cards.find(c=>c.id===id)).filter((c):c is Card=>!!c),...unselected];return <section className="local-repeat-attack" role="dialog"><b>🔮 หยั่งรู้ฟ้าดิน — จัดลำดับกองจั่ว (บนลงล่าง · ใบบนสุดจั่วก่อน)</b><small className="local-peek-hint">กดไพ่เพื่อเลื่อนขึ้นกลุ่ม “บน” ตามลำดับที่กด · ใบที่ไม่ได้กดจะไหลลงล่าง</small>{countdownBadge}<div className="local-force-cards local-peek-list">{ordered.map((c,i)=>{const sel=peekOrder.includes(c.id);return <button key={c.id} className={sel?'selected-card':''} onClick={()=>toggle(c.id)}><b>{i+1}.</b> {c.name} ({c.number}{suitTx(c.suit)}) <em>{sel?'บน':'ล่าง'}{i===0?' · จั่วก่อน':''}</em></button>;})}</div><div className="mock-response-actions"><button onClick={()=>{emit('skill:peek-resolve',{topIds:peekOrder,bottomIds});setPeekOrder([]);}}>ยืนยันการจัดเรียง (บน {peekOrder.length}/ล่าง {bottomIds.length})</button><button className="mock-muted-button" onClick={()=>{emit('skill:peek-resolve',{topIds:cards.map(c=>c.id),bottomIds:[]});setPeekOrder([]);}}>วางทั้งหมดไว้บน (ตามเดิม)</button></div></section>;})()}
  ...
  {detailCard&&(()=>{const info=cardInfo(detailCard);return <div className="modal-backdrop" onClick={()=>setDetailCard(undefined)}><section className="card-detail" onClick={e=>e.stopPropagation()}><button className="modal-close" onClick={()=>setDetailCard(undefined)}>×</button><span className={`card-rank mock-card-suit-${suitColor(detailCard.suit)}`}>{detailCard.number} {suitTx(detailCard.suit)}</span><h2>{detailCard.name}</h2>{detailCard.image&&<img className="card-detail-art" src={detailCard.image} alt={detailCard.name}/>}<p><b>ประเภท:</b> {cardTypeLabel(detailCard)}</p>{(detailCard.cardType==='weapon'||detailCard.equipmentSlot==='weapon')&&detailCard.effectParams?.range?<p className="card-detail-range">🎯 ระยะโจมตี {detailCard.effectParams.range}</p>:null}<p>{info?.desc||detailCard.description||'ยังไม่มีคำอธิบาย'}</p>{info?.use&&<p className="card-detail-use"><b>เมื่อไหร่:</b> {info.use}</p>}</section></div>;})()}
  ...
  {judgmentBanner&&(()=>{const jp=game.players.find(p=>p.id===judgmentBanner.playerId);return <div className="local-judgment-banner" role="status"><span className="local-judgment-title">⚖ การตัดสิน — {judgmentBanner.trickName}</span><div className="local-judgment-card"><span>{judgmentBanner.cardNumber} {suitTx(judgmentBanner.cardSuit)}</span><b>{judgmentBanner.cardName}</b></div><span className="local-judgment-result">{charName(jp)}: {judgmentBanner.result}</span></div>;})()}
  {tableBanner&&<div className="local-table-flash" role="status"><span className="local-table-flash-icon">{tableBanner.icon}</span><div className="local-table-flash-body"><b>{tableBanner.title}</b>{tableBanner.detail&&<span>{tableBanner.detail}</span>}</div>{tableBanner.card&&<div className={`local-judgment-card local-suit-${suitColor(tableBanner.card.suit)}`}><span>{tableBanner.card.number} {suitTx(tableBanner.card.suit)}</span><b>{tableBanner.card.name}</b></div>}</div>}
