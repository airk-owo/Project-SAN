 ...
 return <main className={isPlaying?`mock-game-page local-game-page mock-count-${game.players.length}`:'game-page'}>
  {isPlaying?
   <header className="mock-game-header">
    <div><h1>ยุทธพิชัยสามก๊ก</h1><div className="local-turn-status"><b>ตา: {game.players.find(p=>p.id===game.turn?.activePlayerId)?.username||'—'}</b><span>{game.turn?.phase}{isDrawPhase&&(game.turn?.drawnThisTurn??0)>0?` (จั่ว ${game.turn?.drawnThisTurn??0}/2)`:''}</span></div></div>
   ...
   <header className="local-pregame-header"><h1 className="game-title">ยุทธพิชัยสามก๊ก</h1><span className="phase">{game.isSpectator?'Spectator · ':''}{game.phase}</span>{navbar}</header>}
  {navPanels}
  ...
  {drawPreview&&drawPreview.length>0&&<div className="local-draw-preview" role="status" onClick={()=>setDrawPreview(null)}>
    ...
    <p className="local-draw-preview-label">🎴 จั่วได้ {drawPreview.length} ใบ · แตะเพื่อปิด</p>
   </div>}
  ...
  {isPlaying&&<section className="mock-current-player">
   ...
   <div className="mock-your-turn">{myOwedDraws>0?<strong>⬆ คุณได้รับไพ่ {myOwedDraws} ใบ — กดกองจั่วเพื่อรับ</strong>:selectedAttackId||selectedDiscardId||selectedStealId?<strong>เลือกผู้เล่นเป้าหมายบนโต๊ะ</strong>:isDrawPhase?<strong>⬆ กดกองจั่วบนโต๊ะเพื่อจั่วไพ่</strong>:isMyTurn?<strong className="local-turn-mine">🟢 ตาของคุณ</strong>:(()=>{const ap=game.players.find(p=>p.id===(game.turn?.activePlayerId||game.currentPlayerId));return <span className="local-turn-other">ตาของ <b>{ap?.username||'—'}</b></span>;})()}</div>
