...
export default function Home(){
 ...
 return <>{navbar}<main className={isPlaying?`mock-game-page local-game-page mock-count-${game.players.length}`:'game-page'}>
  ...
  {isPlaying&&<section className="mock-match-layout">
   <section className="mock-table-stage" data-density="large"><div className="mock-table-surface"><div className="mock-table-pattern">三國</div>
    <section className="mock-piles">
     ...
     <article className="mock-pile"><div className={`mock-discard${topDiscard?' local-suit-'+suitColor(topDiscard.suit):''}`}>{topDiscard?<>{topDiscard.name}<br/><span>{topDiscard.number} {topDiscard.suit}</span></>:'—'}</div><b>กองทิ้ง</b><small>{game.discard.length} ใบ</small></article>
    </section>
  ...
  {detailCard&&(()=>{const info=cardInfo(detailCard);return <div className="modal-backdrop" onClick={()=>setDetailCard(undefined)}><section className="card-detail" onClick={e=>e.stopPropagation()}><button className="modal-close" onClick={()=>setDetailCard(undefined)}>×</button><span className={`card-rank mock-card-suit-${suitColor(detailCard.suit)}`}>{detailCard.number} {detailCard.suit}</span><h2>{detailCard.name}</h2>{detailCard.image&&<img className="card-detail-art" src={detailCard.image} alt={detailCard.name}/>}<p><b>ประเภท:</b> {cardTypeLabel(detailCard)}</p><p>{info?.desc||detailCard.description||'ยังไม่มีคำอธิบาย'}</p>{info?.use&&<p className="card-detail-use"><b>เมื่อไหร่:</b> {info.use}</p>}</section></div>;})()}
