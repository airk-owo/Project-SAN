...
export default function Home(){
 ...
 return <>{navbar}<main className={isPlaying?`mock-game-page local-game-page mock-count-${game.players.length}`:'game-page'}>
  ...
  {isPlaying&&game.pendingPeek&&game.pendingPeek.playerId===game.viewerId&&(()=>{const cards=game.pendingPeek.cards;const toggle=(id:string)=>setPeekOrder(prev=>prev.includes(id)?prev.filter(x=>x!==id):[...prev,id]);const bottomIds=cards.filter(c=>!peekOrder.includes(c.id)).map(c=>c.id);return <section className="local-repeat-attack" role="dialog"><b>🔮 หยั่งรู้ฟ้าดิน: กดไพ่ตามลำดับที่จะวางบนสุด (ใบแรกจั่วก่อน) — ที่ไม่ได้เลือกจะลงล่างกอง</b>{countdownBadge}<div className="local-force-cards">{cards.map(c=>{const idx=peekOrder.indexOf(c.id);return <button key={c.id} className={idx>=0?'selected-card':''} onClick={()=>toggle(c.id)}>{idx>=0?`บน #${idx+1} · `:'ล่าง · '}{c.name} ({c.number}{c.suit})</button>;})}</div><div className="mock-response-actions"><button onClick={()=>{emit('skill:peek-resolve',{topIds:peekOrder,bottomIds});setPeekOrder([]);}}>ยืนยันการจัดเรียง (บน {peekOrder.length}/ล่าง {bottomIds.length})</button><button className="mock-muted-button" onClick={()=>{emit('skill:peek-resolve',{topIds:cards.map(c=>c.id),bottomIds:[]});setPeekOrder([]);}}>วางทั้งหมดไว้บน (ตามเดิม)</button></div></section>;})()}
