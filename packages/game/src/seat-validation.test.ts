...
function selectSeat(game: GameState, userId: string, seatInput: unknown): void {
  ...
  if (game.phase !== "waiting")
    throw new Error("เกมเริ่มแล้ว ไม่สามารถเปลี่ยนที่นั่งได้");
  if (!Number.isInteger(seat) || seat < 1 || seat > 10)
    throw new Error("กรุณาเลือกที่นั่ง 1–10");
...
function selectRandomSeat(game: GameState, userId: string): number {
  ...
  const empty = Array.from({ length: 10 }, (_, i) => i + 1).filter(
    (s) => !occupied.has(s),
  );
...
function moveToSpectator(game: GameState, userId: string): void {
  if (game.phase !== "waiting")
    throw new Error("เกมเริ่มแล้ว ไม่สามารถกลับเป็นผู้ชมได้");
...
describe("seat:select – occupancy", () => {
  ...
  it("allows a player to re-select their own current seat", () => {
    ...
    assert.equal(
      ...
      false,
      "ready should reset on re-selection",
    );
  });
...
describe("seat:select – spectator-to-player conversion", () => {
  it("removes user from spectator list when they take a seat", () => {
    ...
    selectSeat(game, "g", 5);
    assert.equal(
      game.spectators.some((s) => s.id === "g"),
      false,
    );
    assert.equal(
      game.players.some((p) => p.id === "g" && p.seatIndex === 5),
      true,
    );
  });
