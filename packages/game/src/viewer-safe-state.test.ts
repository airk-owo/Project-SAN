...
function assignCharacters(game: GameState): void {
  game.players.forEach((p, i) => {
    ...
    p.maxHp =
      char.hp + (p.role === "emperor" && game.players.length !== 4 ? 1 : 0);
    p.hp = p.maxHp;
  });
}
...
describe("viewer-safe state – role visibility", () => {
  ...
  it("viewer cannot see another player's unrevealed role", () => {
    ...
    const hidden = game.players.find(
      (p) => p.id !== viewer.id && !p.roleRevealed,
    )!;
    const view = createPublicGameState(game, viewer.id);
...
describe("viewer-safe state – hand visibility", () => {
  ...
  it("viewer sees only public card fields (no effectParams)", () => {
    ...
    const view = createPublicGameState(game, viewer.id);
    const card = view.players.find((p) => p.id === viewer.id)!
      .hand[0] as Record<string, unknown>;
    assert.ok("id" in card && "name" in card && "suit" in card);
    assert.ok(
      !("effectParams" in card),
      "effectParams should not be in public hand view",
    );
  });
  ...
  it("a weapon in hand exposes only its attack range", () => {
    ...
    const view = createPublicGameState(game, viewer.id);
    const card = view.players.find((p) => p.id === viewer.id)!
      .hand[0] as Record<string, any>;
    assert.equal(
      card.effectParams?.range,
      3,
      "weapon attack range is visible for the detail view",
    );
  });
...
describe("viewer-safe state – character visibility", () => {
  it("viewer sees their own character", () => {
    ...
    const view = createPublicGameState(game, viewer.id);
    assert.equal(
      view.players.find((p) => p.id === viewer.id)?.character?.id,
      "cv",
    );
  });
  ...
  it("emperor's character is visible to all once confirmed", () => {
    ...
    const view = createPublicGameState(game, nonEmperor.id);
    assert.equal(
      view.players.find((p) => p.id === emperor.id)?.character?.id,
      "emp",
    );
  });
  ...
  it("other non-emperor characters are hidden during character selection", () => {
    ...
    const viewer = game.players.find(
      (p) => p.id !== rebel.id && p.role !== "emperor",
    )!;
    const view = createPublicGameState(game, viewer.id);
    assert.equal(
      view.players.find((p) => p.id === rebel.id)?.character,
      undefined,
    );
  });
...
describe("viewer-safe state – character options", () => {
  it("viewer sees their own characterOptions", () => {
    const game = makeCharacterSelectGame(4);
    const viewer = game.players[0]!;
    viewer.characterOptions = [
      makeCharacter("opt1", "Option1"),
      makeCharacter("opt2", "Option2"),
    ];
    const view = createPublicGameState(game, viewer.id);
    assert.equal(
      view.players.find((p) => p.id === viewer.id)?.characterOptions.length,
      2,
    );
  });
  ...
  it("other players' characterOptions are empty", () => {
    ...
    const viewer = game.players[0]!;
    const other = game.players[1]!;
    other.characterOptions = [
      makeCharacter("o1", "O1"),
      makeCharacter("o2", "O2"),
    ];
    const view = createPublicGameState(game, viewer.id);
    assert.deepEqual(
      view.players.find((p) => p.id === other.id)?.characterOptions,
      [],
    );
  });
});
...
describe("viewer-safe state – deck and draw pile", () => {
  it("deck is replaced by a length-only summary", () => {
    ...
    assert.equal(
      typeof (view as unknown as Record<string, unknown>).deck,
      "object",
    );
...
describe("viewer-safe state – response window privacy", () => {
  it("cardInstanceId is stripped from responseWindow responses for non-participants", () => {
    ...
    const responseInView = view.responseWindow?.responses[0] as
      | Record<string, unknown>
      | undefined;
    assert.ok(responseInView, "response should still appear in the window");
    assert.ok(
      !("cardInstanceId" in (responseInView ?? {})),
      "cardInstanceId must be stripped",
    );
  });
