...
// ไฟล์นี้ default เป็น trick + effectParams ว่าง (ต่างจาก helper กลาง)
const makeCard = (
  id: string,
  effect: string,
  cardType: string = "trick",
  extra: Partial<Card> = {},
): Card => baseCard(id, effect, cardType, { effectParams: {}, ...extra });
const negateCard = (id: string): Card => makeCard(id, "negate_trick_effect");
const indulgence = (id: string): Card =>
  makeCard(id, "delayed_skip_play_phase", "delayed_trick");
const lightning = (id: string): Card =>
  makeCard(id, "delayed_lightning_judgment", "delayed_trick");
...
describe("Negate integration – newly-routed tricks open a negate window", () => {
  ...
  it("negating an Indulgence prevents placement and discards it", () => {
    ...
    assert.ok(
      !game.players
        .find((p) => p.id === "p1")!
        .decisionArea.some((c) => c.id === "ind"),
      "not placed",
    );
    assert.ok(
      game.discard.some((c) => c.id === "ind"),
      "trick discarded",
    );
  ...
  it("Lightning placement cannot be negated — it enters the decision area directly", () => {
    ...
    assert.equal(
      game.responseWindow,
      null,
      "no negate window on lightning placement",
    );
    assert.ok(
      game.players
        .find((p) => p.id === "p0")!
        .decisionArea.some((c) => c.id === "lig"),
      "placed directly (negatable only at judgment)",
    );
  });
  ...
  it("negating Harvest reveals nothing and opens no pick window", () => {
    ...
    game.players.find((p) => p.id === "p0")!.hand = [
      makeCard("h", "reveal_and_draft_cards"),
    ];
    game.deck = [
      attackCard("a"),
      attackCard("b"),
  ...
  it("negating Borrowed Knife stops the coercion", () => {
    const game = makeGame();
    game.players.find((p) => p.id === "p0")!.hand = [
      makeCard("c1", "coerce_attack_or_take_weapon"),
    ];
