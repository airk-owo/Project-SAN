...
import type { Card, CardInstance, EquipmentSlots, GamePhase, GameState, GameStatus, HiddenHandSelection, Player, TurnPhase } from './types.js';
...
export function grantDraws(state:GameState,playerId:string,count:number){if(count<=0)return;state.pendingDraws={...state.pendingDraws,[playerId]:owedDraws(state,playerId)+count};}
...
export const numberParam=(card:Card,key:string,fallback:number)=>{const value=card.effectParams[key];return typeof value==='number'?value:fallback};
export const characterName=(player:Player)=>player.username||player.character?.name||'ผู้เล่น';
export const findHandCard=(player:Player,cardInstanceId:string)=>player.hand.find((card,index)=>card.id===cardInstanceId||toCardInstance(card,index).instanceId===cardInstanceId);
...
export const logAction=(state:GameState,type:string,message:string,actorId?:string,targetId?:string,cardId?:string)=>state.log.push({id:crypto.randomUUID(),at:new Date().toISOString(),type,message,actorId,targetId,cardId});
export const moveToDiscard=(state:GameState,card:Card,setLastPlayed=true)=>{state.discard.push(card);state.discardPile.push(toCardInstance(card,state.discard.length-1));if(setLastPlayed)state.lastPlayedCard=card};
export type RuntimeEquipmentSlot=keyof EquipmentSlots<Card>;
export const equipmentSlotForCard=(card:Card):RuntimeEquipmentSlot|undefined=>{switch(card.cardType){case 'weapon':return 'weapon';case 'armor':return 'armor';case 'offensive_mount':return 'offensiveMount';case 'defensive_mount':return 'defensiveMount';default:break;}switch(card.equipmentSlot){case 'weapon':return 'weapon';case 'armor':return 'armor';case 'offensive_mount':return 'offensiveMount';case 'defensive_mount':return 'defensiveMount';default:return undefined;}};
...
export const recordJudgment=(state:GameState,playerId:string,trickName:string,judged:Card|null,result:string)=>{state.lastJudgment={playerId,trickName,cardName:judged?.name??'—',cardNumber:judged?.number??'',cardSuit:judged?.suit??'',result,at:new Date().toISOString()};};
/** ประกาศเหตุการณ์กลางโต๊ะให้ทุกคนเห็น (แบนเนอร์ชั่วคราว) — ใครทำอะไรสำคัญก็เรียกอันนี้ */
export const flashTable=(state:GameState,icon:string,title:string,detail?:string,card?:{name:string;number:string;suit:string})=>{state.tableFlash={icon,title,detail,card:card?{name:card.name,number:card.number,suit:card.suit}:undefined,at:new Date().toISOString()};};
