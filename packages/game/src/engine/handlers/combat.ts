...
import type { Card, EffectParams, GameState, Player, WinningSide } from '../types.js';
import { characterName, findHandCard, getAlivePlayersInSeatOrder, getBaseDistanceBetweenPlayers, getPlayerById, grantDraws, logAction, moveToDiscard, numberParam, reshuffleDiscardIntoDrawPile, synchronizeGameState, toCardInstance, type RuntimeEquipmentSlot } from '../state.js';
...
import { assertForcedAttackTarget, attackDamageBonus, attackDodgesRequired, cardActsAs, dispatchGameEvent, hasCharacterSkill, isImmuneToAttack } from './character-skills.js';
import { areOppositeGenders, attackIgnoresTargetArmor, getAttackRange, hasDamageDestroyTargetMount, hasDiscardTwoForceAttackDamage, hasOppositeGenderAttackChoice, hasRepeatAttackAfterDodge, hasReplaceDamageWithDiscardTwo, hasUnlimitedAttack } from './equipment.js';
...
export function openDyingRescueWindow(state:GameState,dyingPlayerId:string,killerId?:string,sourceActionId=state.currentAction?.actionId||crypto.randomUUID()){
...
export function openAttackDodgeWindow(state:GameState,attacker:Player,target:Player,actionId:string,cardId:string,damage:number,noDodge=false){
