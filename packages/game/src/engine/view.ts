...
export type PublicPlayerState = Omit<Player, "hand" | "characterOptions"> & {
  hand: PublicCardView[];
  handCount: number;
  characterOptions: Character[];
};
export type PublicResponseRecord = Omit<ResponseRecord, "cardInstanceId">;
export type PublicResponseWindow = Omit<ResponseWindow, "responses"> & {
  responses: PublicResponseRecord[];
};
export type PublicGameState = Omit<
  GameState,
  "players" | "deck" | "drawPile" | "responseWindow"
> & {
  viewerId: string;
  isSpectator: boolean;
  deck: { length: number };
  drawPileCount: number;
  responseWindow: PublicResponseWindow | null;
...
export function createPublicGameState(
  ...
  const publicPendingLegacy = pendingLegacy
    ? pendingLegacy.ownerId === viewerId
      ? pendingLegacy
      : { ownerId: pendingLegacy.ownerId, cards: [] }
    : undefined; // คำสั่งเสีย: only กุยแก sees the revealed cards
  const publicResponseWindow = responseWindow
    ? {
        ...responseWindow,
        responses: responseWindow.responses.map(
          ({ cardInstanceId: _cardInstanceId, ...response }) => response,
        ),
      }
    : null;
  const publicPendingPeek = pendingPeek
    ? pendingPeek.playerId === viewerId
      ? pendingPeek
      : { playerId: pendingPeek.playerId, cards: [] }
    : undefined; // peeked cards only visible to จูกัดเหลียง
  // เก็บเกี่ยวยุ้งฉาง: only the current picker sees the remaining cards; others see face-down placeholders.
  const publicPendingHarvest = pendingHarvest
    ? responseWindow?.currentResponderId === viewerId
      ? pendingHarvest
      : {
          ...pendingHarvest,
          revealed: pendingHarvest.revealed.map(
            (c) =>
              ({
                id: c.id,
                name: "ไพ่ปิด",
  ...
  const pub = {
    ...publicState,
    pendingPeek: publicPendingPeek,
    pendingHarvest: publicPendingHarvest,
    pendingLegacy: publicPendingLegacy,
    deck: { length: deck.length },
    drawPileCount: drawPile.length,
    responseWindow: publicResponseWindow,
    viewerId,
    isSpectator: state.spectators.some((s) => s.id === viewerId),
    players: players.map((p) => ({
      ...p,
      role:
        p.id === viewerId || p.roleRevealed || p.role === "emperor"
          ? p.role
          : undefined,
      character:
        p.id === viewerId || p.role === "emperor" || allCharactersChosen
          ? p.character
          : undefined,
      ...
      hand:
        p.id === viewerId
          ? p.hand.map((card) => ({
              ...
              effect: card.effect,
              equipmentSlot: card.equipmentSlot,
              ...(card.cardType === "weapon" && card.effectParams?.range
                ? { effectParams: { range: card.effectParams.range } }
                : {}),
            }))
          : [],
...
function applyClassicCardNames<T>(view: T): T {
  ...
  const walk = (v: unknown): void => {
    ...
    if (v && typeof v === "object") {
      ...
      if (typeof o.name === "string" && "oldName" in o && o.oldName)
        o.name = o.oldName as string;
