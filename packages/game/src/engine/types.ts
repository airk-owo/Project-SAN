...
export type CardInstance = {
  ...
  rank?: string;
  color?: "red" | "black";
  backendEffectKey?: string;
  effectParams?: Record<string, unknown>;
};
export type EquipmentSlots<T = CardInstance> = {
  weapon: T | null;
  armor: T | null;
  offensiveMount: T | null;
  defensiveMount: T | null;
};
...
export type ResponseWindow = {
  ...
  allowedResponseEffectKeys: string[];
  responses: ResponseRecord[];
  status: "open" | "resolved" | "cancelled";
  createdAt: string;
  dyingPlayerId?: string;
  dyingKillerId?: string;
  responderQueue?: string[];
  attackDamage?: number;
  duelResponderAttacks?: number;
};
export type CurrentAction = {
  ...
  effectKey: string;
  targetIds: string[];
  status: "declared" | "resolving" | "resolved" | "cancelled";
  createdAt: string;
};
export type TargetRules = {
  ...
  maxTargets: number;
  allowSelf?: boolean;
  maxDistance?: number | "attack";
};
export type TargetedCardAction = {
...
export type TargetCardSelection =
  ...
  | { zone: "decision_area"; cardInstanceId: string };
...
export type PlayerState = PlayerIdentity & {
  ...
  decisionArea: CardInstance[];
  handLimitOverride?: number | null;
  flags: string[];
};
...
export type EventSubscriber = {
  ...
  event: GameEventName;
  priority?: number;
  handle: (state: GameState, event: GameEvent) => void;
};
export type EffectResolverContext = {
  state: GameState;
  actor: Player;
  card: Card;
  target?: Player;
...
export type PendingRepeatAttack = {
  attackerId: string;
  targetId: string;
  weaponName: string;
};
...
export type PendingReplaceDamage = {
  attackerId: string;
  targetId: string;
  damage: number;
  weaponName: string;
};
export type PendingTwinSwords = {
  attackerId: string;
  targetId: string;
  actionId: string;
  attackCardId: string;
...
export type PendingJudgment = {
  playerId: string;
  trickEffect: "delayed_skip_play_phase" | "delayed_lightning_judgment";
  trickName: string;
  trickCardId: string;
...
export type GameState = {
  ...
  suspendedResponseWindow?: ResponseWindow;
  pendingRepeatAttack?: PendingRepeatAttack;
  pendingDestroyMount?: PendingDestroyMount;
  pendingForceAttackDamage?: PendingForceAttackDamage;
  pendingReplaceDamage?: PendingReplaceDamage;
  pendingTwinSwords?: PendingTwinSwords;
  pendingCoerce?: PendingCoerce;
  ...
  pendingDischord?: PendingDischord;
  pendingAllyAssist?: PendingAllyAssist;
  pendingDraws?: Record<string, number>;
  unarmedPowerActive?: boolean;
  benevolenceGivenThisTurn?: number;
  arrogancePenalty?: boolean;
  lossTracking?: Record<string, { hand: number; equip: string[] }>;
  lastJudgment?: {
    playerId: string;
    trickName: string;
    cardName: string;
  ...
  phase: GamePhase;
  winner?: WinningSide;
  cardNameVersion?: CardNameVersion;
  players: Player[];
  ...
  log: GameLog[];
  pendingRoleComposition?: Record<Role, number>;
  pendingAction?: PendingAction;
  attacksThisTurn: number;
  pendingTrickResolution?: {
    effectKey: string;
    targetId?: string;
